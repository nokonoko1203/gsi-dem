    ...
    pub fn process_all_tiles(&self) -> Result<Vec<DemTile>> {
        ...
        for (name, contents) in xml_files {
            ...
            match parser::parse_dem_xml_from_bytes(&contents) {
                Ok(tile) => {
                    debug!("Successfully parsed tile with mesh code: {:?}", 
...
impl MergedDemTile {
    pub fn from_tiles(mut tiles: Vec<DemTile>) -> Result<Self> {
        ...
        let min_lon = tiles.iter()
            ...
            .unwrap();
        let max_lat = tiles.iter()
            .map(|t| t.origin_lat)
            .max_by(|a, b| a.partial_cmp(b).unwrap())
            .unwrap();
        
        // 実際のデータ範囲を考慮した計算（隣接タイルは1ピクセル重複）
        let max_lon = tiles.iter()
            .map(|t| {
        ...
        debug!("Merging tiles: min_lon={}, max_lon={}, min_lat={}, max_lat={}", min_lon, max_lon, min_lat, max_lat);
        ...
        // タイルをグリッド位置でグループ化
        let mut tile_grid: std::collections::HashMap<(i32, i32), &DemTile> = std::collections::HashMap::new();
        ...
        for (i, tile) in tiles.iter().enumerate() {
            ...
            if i < 20 {  // 最初の20タイルでデバッグ
                debug!("Tile {} grid=({},{}), is_leftmost={}, is_topmost={}, offset=({},{})", 
                    i, grid_x, grid_y, is_leftmost_in_grid, is_topmost_in_grid, 
