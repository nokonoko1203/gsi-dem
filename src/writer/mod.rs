...
impl GeoTiffWriter {
    pub fn new(compress: bool, tiling: bool) -> Self {
        Self { compress, tiling }
    }
    ...
    pub fn write(&self, dem_tile: &DemTile, output_path: &Path) -> Result<()> {
        ...
        // ジオトランスフォームを設定
        dataset.set_geo_transform(&dem_tile.geo_transform())
        ...
        if let Some(epsg) = dem_tile.guess_epsg() {
            ...
            // EPSGコードが推定できない場合は警告
            eprintln!("Warning: Unknown CRS identifier: {}", dem_tile.metadata.crs_identifier);
        }
        
        // バンドにデータを書き込み
        let mut band = dataset.rasterband(1)
            .context("Failed to get raster band")?;
        ...
        // メタデータを設定（オプション）
        dataset.set_metadata_item("MESHCODE", &dem_tile.metadata.meshcode, "")
            .context("Failed to set meshcode metadata")?;
        dataset.set_metadata_item("DEM_TYPE", &dem_tile.metadata.dem_type, "")
...
mod tests {
    ...
    fn test_write_geotiff() {
        ...
        let writer = GeoTiffWriter::new(false, false);
