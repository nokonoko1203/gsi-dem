"""
QGIS Plugin Example for japan DEM Parser

This example demonstrates how to use the japan_dem Python bindings
in a QGIS plugin to parse japan DEM XML files and create raster layers.
"""
...
# Import the japan_dem module
import japan_dem


class japanDemLoaderPlugin:
    """QGIS Plugin for loading japan DEM XML files"""
    ...
    def initGui(self):
        ...
        # Create action
        self.action = QAction("Load japan DEM XML", self.iface.mainWindow())
        self.action.triggered.connect(self.run)

        # Add toolbar button and menu item
        self.iface.addToolBarIcon(self.action)
        self.iface.addPluginToRasterMenu("japan DEM Loader", self.action)

    def unload(self):
        """Remove the plugin menu item and icon"""
        self.iface.removePluginRasterMenu("japan DEM Loader", self.action)
        self.iface.removeToolBarIcon(self.action)

    def run(self):
        """Run method that loads the japan DEM XML file"""
        # Get file path from user
        file_path, _ = QFileDialog.getOpenFileName(
            self.iface.mainWindow(),
            "Select japan DEM XML file",
            "",
            "XML files (*.xml)",
        )
        ...
        try:
            # Parse the DEM XML file using japan_dem
            dem_tile = japan_dem.parse_dem_xml(file_path)
            ...
            output_path = os.path.join(
                temp_dir, f"dem_{dem_tile.metadata.mesh_code}.tif"
            )
            ...
            QMessageBox.critical(
                self.iface.mainWindow(), "Error", f"Failed to load DEM: {str(e)}"
            )
    ...
    def _create_geotiff(self, dem_tile, output_path):
        ...
        try:
            ...
            data_array = np.full(
                (dem_tile.rows, dem_tile.cols), -9999.0, dtype=np.float32
            )
            ...
            raise RuntimeError(
                "GDAL Python bindings not found. Please install python-gdal."
            )


# Example usage in QGIS Python console:
"""
# After installing the japan_dem module with maturin:
# pip install maturin
...
dem_tile = japan_dem.parse_dem_xml('/path/to/your/dem.xml')
